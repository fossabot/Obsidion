...
class MyHelpCommand(commands.HelpCommand):
    ...
    async def generate_embed(self, ctx: commands.Context, prefix: str = None) -> None:
        """Generate embed."""
    ...
    async def send_command_help(self, command: Command) -> None:
        ...
        embed.description = (
            f"`Syntax: {prefix}{command.qualified_name} "
            f"{command.signature}`\n\n{command.help}"
        )
        if len(command.aliases) > 0:
            embed.add_field(
    ...
    async def send_group_help(self, group: commands.Group) -> None:
        ...
        embed.set_footer(
            text=(
                ...
                f"on a command. You can also type {self.context.prefix}help "
